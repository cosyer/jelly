...
window.addEventListener("timeLineChangeEnd", () =>
  setTimeout(() => clock(), 1000)
);

function init() {
  document
    .querySelector(".el_psy_congroo")
    .addEventListener("click", () => timeLineChangeStart());
    ...
    if (_currentRandom < _allRandom) {
      ...
      _imgWrapperNode.forEach((item, idx) => {
        if (_currentRandom > _allRandom - 5 && idx === 0) {
          replaceNode(
            _imgWrapperNode[0],
            _imgAll[Math.floor(Math.random() * 3)].cloneNode()
          );
        } else if (idx !== 1) {
          replaceNode(
            item,
            _imgAll[Math.floor(Math.random() * 10)].cloneNode()
          );
        } else {
