...
function calculate() {
  if (
    ...
    (bird.bx + bird.bwidth > up_pipe.px &&
      bird.by > up_pipe.py &&
      bird.bx + bird.bwidth < up_pipe.px + up_pipe.pwidth &&
      bird.by < up_pipe.py + up_pipe.pheight) ||
    (bird.bx + bird.bwidth > down_pipe.px &&
      bird.bx + bird.bwidth < down_pipe.px + down_pipe.pwidth &&
      bird.by + bird.bheight > down_pipe.py &&
  ...
  if (score > 0) {
    score % 10 != 0
      ...
      : ctx.fillText("Great!" + score, 120, 100);
  }
}
