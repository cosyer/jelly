...
Jx().$package(function(J) {
  ...
  if (J.browser.ie /*document.attachEvent*/) {
    //这里不能用特性判断, 否则opera也会使用这个方法绑定事件
  ...
  // addEventListener( ) and removeEventListener( ).
    ...
    removeOriginalEventListener = function(element, eventType, handler) {
      if (eventType) {
        if (arguments.length == 3) {
          //修复传入了第三个参数,但是第三个参数为 undefined 的问题
          if (handler) {
  ...
  customEvent = {
    ondrag: function(element, eventType, handler) {
      ...
      var onElMousedown = function(e) {
        if (!J.browser.mobileSafari && e.button !== 0) {
          //非左键点击直接return
          return;
        }
        var touch;
      ...
      customEventHandlers.push({
        element: element,
        eventType: eventType,
        handler: handler,
        actions: [onElMousedown, onElMouseup]
      });
    },
    offdrag: function(element, eventType, handler) {
      for (var i in customEventHandlers) {
        if (
          customEventHandlers[i].handler == handler &&
          customEventHandlers[i].element == element &&
          customEventHandlers[i].eventType == eventType
        ) {
          if (J.browser.mobileSafari) {
            $E.removeEventListener(
              element,
              "touchstart",
              customEventHandlers[i].actions[0]
            );
            $E.removeEventListener(
              element,
              "touchend",
              customEventHandlers[i].actions[1]
            );
          } else {
            $E.removeEventListener(
              element,
    ...
    oncustomclick: function(element, eventType, handler, options) {
      //@ longTouchable 是否触发长按事件 add by ip
      var _oldX,
        _oldY,
        isMove = false,
        isClicked = false,
      ...
      var onElClick = function(e) {
        ...
        longTouchTimer = null;
        isClicked = true;
        if (!J.browser.mobileSafari && mouseButton !== 0) {
          //非左键点击直接return
          return;
        }
        var touch;
      ...
      customEventHandlers.push({
        ...
        eventType: eventType,
        handler: handler,
        actions: [onElMousedown, onElMouseMove, onElMouseup, onElClick]
      });
    },
      ...
      for (var i in customEventHandlers) {
        if (
          ...
          if (J.browser.mobileSafari) {
            ...
            $E.removeOriginalEventListener(
              element,
              "touchend",
              customEventHandlers[i].actions[3]
            );
          } else {
            $E.removeEventListener(
              element,
            ...
            $E.removeEventListener(
              element,
              "mouseup",
              customEventHandlers[i].actions[2]
            );
            $E.removeOriginalEventListener(
              element,
              "click",
    ...
    oninput: function(element, eventType, handler) {
      if (J.browser.ie) {
        ...
        if (J.browser.ie == 9) {
          //fix ie9 bug, can not fire when characters are deleted
