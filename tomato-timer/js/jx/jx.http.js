  ...
  ajax = function(uri, option) {
    ...
    if (option.method === "GET") {
      ...
      httpRequest.setRequestHeader(
        "Content-Type",
        option.contentType || "text/plain;charset=UTF-8"
      );
      httpRequest.send();
  ...
  comet = function(uri, option) {
    ...
    if (J.browser.ie) {
      ...
      connection.setAttribute(
        "src",
        uri + "?callback=window.parent._parent." + option.callback
      );
  ...
  var formSendIframePool = {
    ...
    init: function(len) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        $D.hide(divEl);
        divEl.innerHTML =
          '<iframe id="RPCService_hIframe_' +
          i +
          '" name="RPCService_hIframe_' +
          i +
          '" src="' +
          alloy.CONST.MAIN_URL +
          'domain.html"></iframe>';
